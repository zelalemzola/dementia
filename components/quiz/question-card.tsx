...
export function QuestionCard({ question, onAnswer, selectedAnswer }: QuestionCardProps) {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-xl border border-border bg-card p-4 sm:p-5 shadow-2xl">
        {/* Decorative elements */}
        <div className="absolute -right-20 -top-20 h-32 w-32 rounded-full bg-primary/10 blur-3xl" />
        <div className="absolute -left-20 -bottom-20 h-32 w-32 rounded-full bg-accent/10 blur-3xl" />
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground">Question {question.id}</span>
        </div>

        {/* Question text */}
        <h2 className="relative mb-4 text-base sm:text-lg font-semibold leading-snug text-foreground">
          {question.text}
        </h2>
        ...
        <div className={cn(
          "grid gap-2",
          options.length > 3 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"
        )}>
          {options.map((option) => (
            <button
              key={option}
              ...
              className={cn(
                "group relative overflow-hidden rounded-lg border p-3 text-left transition-all duration-300",
